...
def build_agents_block(root: Path) -> str:
    return (
        ...
        "Use `$model-orchestrator` only when delegation is actually worth it.\n\n"
        "Prefer this split:\n"
        "- Keep final planning, risky decisions, and user-facing synthesis in Codex.\n"
        "- Use Gemini worker lanes for bounded, reversible subtasks when that saves time or tokens.\n"
        "- Do not delegate trivial work or tasks whose value is mostly in integration.\n\n"
        "Default delegated lanes:\n"
        "- `fast_worker`: main external worker for summaries, multimodal inspection, and first-pass analysis\n"
        "- `budget_worker`: cheap preprocessing, clustering, chunk summaries, tagging\n"
        "- `reasoner` or `verifier`: only when evidence conflicts or stakes increase\n\n"
        "Token discipline:\n"
        ...
        "- Prefer Gemini delegation only when it reduces cost or latency without hurting correctness.\n"
        f"{AGENTS_END}\n"
    )
